...
{
    class HeliAdapter:BaseAdapter<Heli>, IFilterable
    {
        private List<Heli> _originalData;
        private List<Heli> _items;
        private readonly Activity _context;

        public HeliAdapter(Activity activity, IEnumerable<Heli> helis)
        {
            _items = helis.OrderBy(x => x.VehicleId).ThenBy(x => x.Nation).ToList();
            _context = activity;

            Filter = new HeliFilter(this);
        }
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            nameView.Text = helis.Name;
            rankViewRow.Text = rank + helis.Rank;
            brViewRow.Text = br + System.String.Format("{0:F1}", helis.BR);
            repairViewRow.Text = repair + helis.RepairCost.ToString();
        ...
        public override Heli this[int position]
        {
        ...
        private class HeliFilter : Filter
        {
            private readonly HeliAdapter _adapter;
            public HeliFilter(HeliAdapter adapter)
            {
                _adapter = adapter;
            }
            ...
            protected override void PublishResults(ICharSequence constraint, FilterResults results)
            {
                using (var values = results.Values)
                    _adapter._items = values.ToArray<Object>()
                        .Select(r => r.ToNetObject<Heli>()).ToList();
